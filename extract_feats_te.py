...
def agg_quantile_feats_categ(df, agg_col):
...
def compute_feats(df, test_feats, tenure_categ_mapper, tenure_int_mapper):
    ...
    df['N_MISSING_OP'] = df[['ON_NET', 'ORANGE', 'TIGO']].isna().sum(axis=1)
    df['N_MISSING_REV'] = df[['MONTANT', 'REVENUE']].isna().sum(axis=1)
    df['N_MISSING_ZONE'] = df[['ZONE1', 'ZONE2']].isna().sum(axis=1)
    df['N_MISSING_PACK'] = df[['TOP_PACK', 'FREQ_TOP_PACK']].isna().sum(axis=1)
    df['N_MISSING_FREQ_DT'] = df[['FREQUENCE_RECH', 'FREQUENCE', 'DATA_VOLUME']].isna().sum(axis=1)
    df['N_MISSING_IDENTITY'] = df[['REGION', 'TENURE']].isna().sum(axis=1)
    ...
    df['UNLIMITED_CALL'] = df["TOP_PACK"].map(lambda x: 1 if "unlimited" in str(x).lower() else 0)
    df['FORFAIT_24h'] = df["TOP_PACK"].map(lambda x: 1 if "24h" in str(x).lower() else 0)
    df['FORFAIT_5d'] = df["TOP_PACK"].map(lambda x: 1 if "5d" in str(x).lower() else 0)
    df['FORFAIT_7d'] = df["TOP_PACK"].map(lambda x: 1 if "7d" in str(x).lower() else 0)
    ...
    for col in ['REGION', 'TENURE', 'TOP_PACK']:
        df[col] = df[col].cat.add_categories("OTHER").fillna("OTHER")

    df['REGION_TENURE'] = df['REGION'].astype(str)+'_'+df['TENURE'].astype(str) 
    df['REGION_TOP_PACK'] = df['REGION'].astype(str)+'_'+df['TOP_PACK'].astype(str)
    df['TENURE_TOP_PACK'] = df['TENURE'].astype(str)+'_'+df['TOP_PACK'].astype(str)
    
    return df

def target_encode(train, test, feature, target):
    t_enc = TargetEncoding()
    train_te, test_te = t_enc.fit(train, test, feature, target)
...
if __name__=='__main__':
    ...
    encode_CB(X_train, X_test, 'REGION','TOP_PACK')
    encode_CB(X_train, X_test, 'REGION','TENURE')
    encode_CB(X_train, X_test, 'TENURE','TOP_PACK')

    X_train, X_test = target_encode(X_train, X_test, 'REGION', 'CHURN')
    X_train, X_test = target_encode(X_train, X_test, 'TENURE', 'CHURN')
    X_train, X_test = target_encode(X_train, X_test, 'TOP_PACK', 'CHURN')
    X_train, X_test = target_encode(X_train, X_test, 'REGION_TENURE', 'CHURN')
    X_train, X_test = target_encode(X_train, X_test, 'REGION_TOP_PACK', 'CHURN')
    X_train, X_test= target_encode(X_train, X_test, 'TENURE_TOP_PACK', 'CHURN')

    X_train.to_pickle(os.path.join(cfg.PATH, 'Train_te.pkl'))
    X_test.to_pickle(os.path.join(cfg.PATH, 'Test_te.pkl')) 